...
function slider_score_meta_box( $object, $box )
{
	...
	ci_metabox_input('ci_cpt_slider_url', __('Slider URL. If someone clicks on a slider item, this is the link that they will be visiting. If you leave it empty, linking for this slide will be disabled.', 'ci_theme'), array('esc_func' => 'esc_url'));
	ci_metabox_input('ci_cpt_slider_video', sprintf(__('Video URL. It must be hosted in one of the <a href="%s">WordPress supported providers</a>.', 'ci_theme'), 'http://codex.wordpress.org/Embeds'), array('esc_func' => 'esc_url'));
...
function slider_save_meta( $post_id, $post ) {
	...
	update_post_meta($post->ID, 'ci_cpt_slider_url', esc_url_raw($_POST['ci_cpt_slider_url']) );
	update_post_meta($post->ID, 'ci_cpt_slider_video', esc_url_raw($_POST['ci_cpt_slider_video']) );
	update_post_meta($post->ID, 'ci_cpt_slider_text', ci_sanitize_checkbox($_POST['ci_cpt_slider_text'], 1) );
}
endif;
